-- ============================================================
-- 链操作记录表（service-chain 专用）
-- 执行方式：在 service-chain 连接的数据库（microservices）中手动执行一次
-- 记录每次上链操作（创建地址/上链/铸造/转账/销毁）及链上返回结果，
-- 供 ChainProcessJob 定时扫描 PROCESSING 状态推进
-- ============================================================

CREATE TABLE IF NOT EXISTS `chain_operate_info` (
      ...
      PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='链操作记录表';
