 ...
 * 脱敏响应体处理
 * <p>
 * 对返回的 UserInfo / InviteRankInfo 中的敏感字段（手机号、昵称等）进行脱敏，
 * 通过 {@link SensitiveUtil} 基于实体字段上的 @SensitiveStrategyXxx 注解完成
 *
 * @author hollis
 */
@ControllerAdvice
public class SensitiveResponseBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType,
        ...
        // 如果返回的对象是 UserInfo/InviteRankInfo，进行脱敏处理
        if (body != null && body instanceof Result) {
            ...
            if (((Result<?>) body).getData() instanceof Collection<?>) {
                ((Result<Collection>) body).setData(SensitiveUtil.desCopyCollection(
                        (Collection) ((Result<?>) body).getData()));
                return body;
            }
